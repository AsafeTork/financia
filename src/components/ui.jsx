...
export const Card = ({ children, className = '', hover = false }) => (
  <div className={'bg-white rounded-2xl shadow-sm border border-gray-100 ' + (hover ? 'card-hover ' : '') + className}>{children}</div>
);
...
export const Inp = ({ label, hint, className = '', ...p }) => (
  <div className={'flex flex-col gap-1.5 ' + className}>
    ...
    <input className="border border-gray-200 rounded-xl px-3 py-2.5 text-sm bg-white transition w-full" {...p}/>
...
export const Sel = ({ label, className = '', children, ...p }) => (
  <div className={'flex flex-col gap-1.5 ' + className}>
    {label && <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide">{label}</label>}
    <select className="border border-gray-200 rounded-xl px-3 py-2.5 text-sm bg-white transition" {...p}>{children}</select>
  </div>
);
...
export const Textarea = ({ label, className = '', ...p }) => (
  <div className={'flex flex-col gap-1.5 ' + className}>
    {label && <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide">{label}</label>}
    <textarea className="border border-gray-200 rounded-xl px-3 py-2.5 text-sm bg-white transition w-full resize-none" rows={6} {...p}/>
  </div>
);
...
export const Empty = ({ icon, title, sub, action, onAction, color }) => (
  ...
  <div className="fixed inset-0 z-50 flex items-center justify-center p-4 anim-fade" style={{background:'rgba(0,0,0,0.5)'}}>
    <div className={'bg-white rounded-2xl flex flex-col w-full ' + (wide ? 'max-w-lg' : 'max-w-sm')} style={{boxShadow:'0 25px 60px rgba(0,0,0,0.2)',maxHeight:'90vh'}}>
      ...
      <div className="flex gap-2 px-6 pb-5 flex-shrink-0 pt-2">
        ...
        <button onClick={onSave} disabled={saving} className="flex-1 text-white rounded-xl py-2.5 text-sm font-semibold hover:opacity-90 flex items-center justify-center gap-2 disabled:opacity-40 transition" style={{background:color}}>
